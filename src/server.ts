...
import { AuthStore, type MetaCredential, authStorage } from "./auth.js";
...
if (process.env.NODE_ENV === "production") {
  ...
  if (process.env.VERCEL && authStorage !== "upstash-redis") throw new Error("Vercel production requires UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN for durable OAuth storage.");
}
...
app.get(route("/health"), (_req, res) => res.json({ ok: true, service: "thedailyflare-meta-ads-mcp", endpoint: `${baseUrl}/mcp`, api_version: apiVersion, storage: authStorage }));
app.get(route("/.well-known/oauth-protected-resource"), (_req, res) => res.json({ resource: `${baseUrl}/mcp`, authorization_servers: [baseUrl], scopes_supported: ["mcp"] }));
app.get(route("/.well-known/oauth-authorization-server"), (_req, res) => res.json({ issuer: baseUrl, authorization_endpoint: `${baseUrl}/oauth/authorize`, token_endpoint: `${baseUrl}/oauth/token`, registration_endpoint: `${baseUrl}/oauth/register`, response_types_supported: ["code"], grant_types_supported: ["authorization_code"], code_challenge_methods_supported: ["S256"], token_endpoint_auth_methods_supported: ["none"] }));
...
app.get(route("/oauth/authorize"), async (req, res) => {
  ...
  const oauthState = await store.createOAuthState({ clientId, redirectUri, state, challenge });
...
app.post(route("/oauth/token"), async (req, res) => {
  ...
  return res.json({ access_token: accessToken, token_type: "Bearer", expires_in: Math.max(60, redeemed.meta.expiresAt - Math.floor(Date.now() / 1000)), scope: "mcp" });
});
...
const mcpHandler = createMcpHandler(ctx => {
  ...
  const server = new McpServer({ name: "thedailyflare-meta-ads", version: "1.3.0", title: "The Daily Flare Meta Ads MCP", description: "Meta advertising management and analytics for The Daily Flare." });
...
app.all(route("/mcp"), async (req, res) => {
  ...
  if (!meta) { res.setHeader("WWW-Authenticate", `Bearer resource_metadata="${baseUrl}/.well-known/oauth-protected-resource"`); return res.status(401).json({ error: "unauthorized", error_description: "Connect this MCP server through OAuth first." }); }
